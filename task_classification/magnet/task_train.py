    ...
    def _read_tsv(cls, input_file, quotechar=None):
        with tf.io.gfile.GFile(input_file, "r") as f:
            ...
            for line in reader:
                ...
                lines.append([line[1], json.loads(line[2])])
            return lines
...
class DataSequence(Sequence):
    ...
    def feature_batch_transform(self, features):
        ...
        return {"input_ids": batch_token_ids,
                "token_type_ids": batch_segment_ids}, np.squeeze(np.array(batch_labels),
                                                                 axis=1)
...
class TFAlbertGAT(TFAlbertPreTrainedModel, TFMultiLabelClassificationLoss):
    """
    ...
    def __init__(self, config: AlbertConfig, *inputs, **kwargs):
        ...
        #     config.gat_in_dim, config.gat_num_hidden, config.gat_heads[0],
        #     config.gat_feat_drop, config.gat_attn_drop, config.gat_negative_slope, False, self.activation))
        # # hidden layers
        # for l in range(1, config.gat_num_layers):
        #     # due to multi-head, the in_dim = num_hidden * num_heads
        #     self.gat_layers.append(GATConv(
        #         config.gat_num_hidden * config.gat_heads[l - 1], config.gat_num_hidden, config.gat_heads[l],
        #         config.gat_feat_drop, config.gat_attn_drop, config.gat_negative_slope, config.gat_residual,
        #         self.activation))
        # # output projection
        # self.gat_layers.append(GATConv(
        #     config.gat_num_hidden * config.gat_heads[-2], config.gat_num_classes, config.gat_heads[-1],
        #     config.gat_feat_drop, config.gat_attn_drop, config.gat_negative_slope, config.gat_residual, None))
...
class ClassificationReporter(Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        for (x_val, y_val) in self.validation_data:
            ...
            # 这里可以设置不同的阈值进行验证
            val_pred.append(np.asarray(val_pred_batch).round())
...
if __name__ == '__main__':
    ...
    parse.add_argument("--loss_type", type=str, default="bce", choices=["bce", "focal_loss"],
    ...
    config.gat_in_dim = 128
    config.gat_num_hidden = 128
    config.gat_heads = [100, 100, 100]
    config.gat_feat_drop = 0
    config.gat_attn_drop = 0
    config.gat_negative_slope = 0
    config.gat_residual = False
    # 加载模型
    model = TFAlbertGAT.from_pretrained(args.bert_model,
                                        config=config
                                        )
    ########模型loss和metric：
    if args.loss_type == "bce":
        model.compile(loss=model.compute_loss, optimizer=Adam(args.lr),
                      metrics=[tf.keras.metrics.BinaryAccuracy(), tf.keras.metrics.AUC()])
    if args.loss_type == "focal_loss":
        model.compile(loss=model.compute_focal_loss, optimizer=Adam(args.lr),
                      metrics=[tf.keras.metrics.BinaryAccuracy(), tf.keras.metrics.AUC()])
    model.summary()
    #######添加callback####
