...
if __name__ == '__main__':
    ...
    parse.add_argument("--output_root", type=str, default="output_v4", help="output dir")
    ...
    checkpoint = tf.keras.callbacks.ModelCheckpoint(os.path.join(model_dir, 'albert_distill'),
                                                    monitor="loss",
                                                    verbose=2,
                                                    save_best_only=True,
                                                    mode='min',
    ...
    tensor_board = tf.keras.callbacks.TensorBoard(log_dir=log_dir, write_graph=True, histogram_freq=1)
    call_backs.append(tensor_board)
    model_dir = os.path.join(args.output_root, "model")
    if not os.path.exists(model_dir):
        os.makedirs(model_dir)

    early_stop = tf.keras.callbacks.EarlyStopping('loss', patience=5, mode='min', verbose=2, restore_best_weights=True)
    ...
    model.summary()
    # #只保存学生模型#############################################
    model.load_weights(os.path.join(model_dir, "albert_distill"))
    model.student.save_weights(os.path.join(model_dir, "student_distll.h5"))
